...
enum Faces_ {ACE_, TWO_, THREE_, FOUR_, FIVE_, SIX_, SEVEN_, EIGHT_, NINE_, TEN_, JACK_, QUEEN_, KING_};
enum Suits_ { CLUB_, DIAMOND_, HEART_, SPADE_ };

class DeckGUI {
public:
	DeckGUI();
	virtual ~DeckGUI();
	Glib::RefPtr<Gdk::Pixbuf> getCardImage( Faces_ f, Suits_ s );   // Returns the image for the specified card.
